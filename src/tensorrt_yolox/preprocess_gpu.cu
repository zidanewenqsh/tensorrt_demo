...
__global__ void preprocess_kernel(const unsigned char* input, float* output, const float* matrix, int width, int height, int w, int h, const float* mean, const float* std) {
    ...
    int index = idy * w + idx;
    // yolox
    output[index] = bilinearInterpolateChannel(input, width, height, x, y, 0, 3);
    output[channelSize + index] = bilinearInterpolateChannel(input, width, height, x, y, 1, 3);
    output[2 * channelSize + index] = bilinearInterpolateChannel(input, width, height, x, y, 1, 3);
}
