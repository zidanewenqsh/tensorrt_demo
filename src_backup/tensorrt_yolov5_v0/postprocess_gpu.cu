...
__global__ void filter_boxes_kernel_withnms(float* boxes, gBox *filtered_boxes, float* d_matrix, int num_boxes, int num_probs, 
...
__global__ void nms(gBox *filtered_boxes, int box_count,  float nms_threshold) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
